...
class KioskController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        ActivityLog::log('visitor_checkin', "Tamu check-in: {$visitor->name} dari {$visitor->institution}", $visitor);

        // Send email notification to the assigned staff
        if ($visitor->staff_id) {
            /** @var User|null $staff */
            $staff = User::find($visitor->staff_id);
            if ($staff && $staff->email) {
                Mail::to($staff->email)->send(new VisitorNotificationMail($visitor));
            }
        }
