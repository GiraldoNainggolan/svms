                    ...
                    <tr class="border-b border-white/[0.03] hover:bg-white/[0.02] transition-colors">
                        <td class="py-3.5 px-6">
                            <div class="flex items-center gap-3">
                                <div class="w-8 h-8 rounded-lg flex items-center justify-center text-xs font-bold
                                    @if($user->role === 'super_admin') bg-gradient-to-br from-amber-500 to-orange-600
                                    @elseif($user->role === 'security') bg-gradient-to-br from-blue-500 to-cyan-600
                        ...
                        <td class="py-3.5 px-6">
                            <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-[11px] font-semibold
                                @if($user->role === 'super_admin') bg-amber-500/10 text-amber-400 border border-amber-500/20
                                @elseif($user->role === 'security') bg-blue-500/10 text-blue-400 border border-blue-500/20
                                @elseif($user->role === 'staff') bg-violet-500/10 text-violet-400 border border-violet-500/20
                                @else bg-slate-500/10 text-slate-400 border border-slate-500/20
                                @endif">
                                <span class="w-1.5 h-1.5 rounded-full
                                    @if($user->role === 'super_admin') bg-amber-400
                                    @elseif($user->role === 'security') bg-blue-400
                                    @elseif($user->role === 'staff') bg-violet-400
                                    @else bg-slate-400
                                    @endif"></span>
