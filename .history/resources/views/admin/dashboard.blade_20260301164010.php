            ...
            <div class="space-y-0">
                @forelse($recentLogs as $log)
                    @php
                        ...
                        $logIconBg = [
                            'create' => 'bg-emerald-500/10', 'delete' => 'bg-red-500/10',
                            'checkin' => 'bg-blue-500/10', 'checkout' => 'bg-amber-500/10',
                            'default' => 'bg-slate-500/10',
                        ][$logType];
                        $logIconText = [
                            'create' => 'text-emerald-400', 'delete' => 'text-red-400',
                            'checkin' => 'text-blue-400', 'checkout' => 'text-amber-400',
                            'default' => 'text-slate-400',
                        ][$logType];
                    @endphp
                    <div class="flex items-start gap-3 py-3 border-b border-white/[0.04] last:border-0">
                        <div class="w-7 h-7 mt-0.5 rounded-lg flex items-center justify-center flex-shrink-0 {{ $logIconBg }}">
                            @if($logType === 'create')
                                <svg class="w-3.5 h-3.5 {{ $logIconText }}" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M12 4.5v15m7.5-7.5h-15" /></svg>
                            @elseif($logType === 'delete')
                                <svg class="w-3.5 h-3.5 {{ $logIconText }}" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M14.74 9l-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 01-2.244 2.077H8.084a2.25 2.25 0 01-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 00-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 013.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 00-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 00-7.5 0" /></svg>
                            @else
                                <svg class="w-3.5 h-3.5 {{ $logIconText }}" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M12 6v6h4.5m4.5 0a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                            @endif
                        </div>
