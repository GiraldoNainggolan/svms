    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
        ...
        <div class="stat-card group">
            <div class="flex items-center justify-between mb-3">
                <div
                    class="w-10 h-10 bg-blue-500/10 rounded-xl flex items-center justify-center group-hover:scale-110 transition-transform">
        ...
        <div class="stat-card group">
            <div class="flex items-center justify-between mb-3">
                <div
                    class="w-10 h-10 bg-emerald-500/10 rounded-xl flex items-center justify-center group-hover:scale-110 transition-transform">
        ...
        <div class="stat-card group">
            <div class="flex items-center justify-between mb-3">
                <div
                    class="w-10 h-10 bg-red-500/10 rounded-xl flex items-center justify-center group-hover:scale-110 transition-transform">
            ...
            <p class="text-3xl font-bold tracking-tight {{ $overdueVisitors > 0 ? 'text-red-400' : '' }}">
                {{ $overdueVisitors }}</p>
        ...
        <div class="xl:col-span-2 space-y-3">
            ...
            @forelse($visitors as $visitor)
                @php
                    ...
                    $statusLabelMap = [
                        'waiting'  => 'Menunggu Staff',
                        'accepted' => 'Diterima Staff',
                        'in'       => 'Di Lokasi',
                        'default'  => $visitor->status,
                    ];
                    $badgeStyle = $statusBadgeMap[$statusType];
                    $statusLabel = $statusLabelMap[$statusType];
                @endphp
                <div
                    class="glass-card p-5 flex items-center justify-between hover:bg-white/[0.06] transition-all {{ $isOverdue ? 'border-red-500/20' : '' }}">
                    <div class="flex items-center gap-4">
                        ...
                        @else
                            <div
                                class="w-12 h-12 rounded-xl bg-gradient-to-br from-slate-700 to-slate-800 flex items-center justify-center text-sm font-bold text-slate-400">
                        ...
                        <div>
                            <div class="flex items-center gap-2">
                                ...
                                <span class="text-[10px] font-semibold px-2 py-0.5 rounded-full border {{ $badgeStyle }}">{{ $statusLabel }}</span>
                                @if ($isOverdue)
                                    <span
                                        class="text-[10px] font-bold text-red-400 bg-red-500/10 px-2 py-0.5 rounded-full">OVERDUE</span>
                                @endif
                            </div>
                            <p class="text-xs text-slate-500">{{ $visitor->institution ?? '-' }} &middot;
                                {{ $visitor->purpose ?? '-' }}</p>
                            <p class="text-[10px] text-slate-600 mt-0.5">
                                Bertemu: {{ $visitor->staff->name ?? '-' }} &middot;
                                Check-in: {{ $visitor->created_at->format('H:i') }}
                                @if ($isOverdue)
                                    <span class="text-red-400/70">&middot;
                                        {{ $visitor->created_at->diffForHumans() }}</span>
                                @endif
                            </p>
                        </div>
                    </div>
                    @if($visitor->status === 'IN')
                        <form method="POST" action="{{ route('security.checkout', $visitor->id) }}"
                            ...
                            @method('PATCH')
                            <button
                                class="bg-amber-500/10 hover:bg-amber-500/20 border border-amber-500/20 text-amber-400 px-4 py-2 rounded-xl text-sm font-semibold transition">
                                Check-out
                            </button>
                        </form>
                    @else
                        <span class="text-[10px] text-slate-600 italic">Belum bisa checkout</span>
                    @endif
                </div>
            @empty
        ...
        <div class="space-y-6">
            ...
            <div class="glass-card p-5">
                ...
                <div class="flex items-center gap-3 mb-4">
                    <div
                        class="w-12 h-12 bg-gradient-to-br from-blue-500 to-cyan-600 rounded-xl flex items-center justify-center text-lg font-bold">
            ...
            <div class="glass-card p-5">
                <div class="flex items-center justify-between mb-4">
                    ...
                    <a href="{{ route('security.activity') }}"
                        class="text-[10px] text-blue-400 hover:text-blue-300 transition font-medium">
                        Lihat Semua &rarr;
                    </a>
                </div>
                ...
                @forelse($logs as $log)
                    ...
                    <div class="flex items-start gap-3 mb-3 last:mb-0">
                        <div
                            class="w-8 h-8 {{ $iconBg }} rounded-lg flex items-center justify-center flex-shrink-0 mt-0.5">
                            @if ($actionType === 'checkout')
                                <svg class="w-4 h-4 {{ $iconText }}" fill="none" stroke="currentColor"
