        ...
        <div class="w-full max-w-xl">
            ...
            <div class="bg-white/[0.06] backdrop-blur-xl rounded-3xl border border-white/10 shadow-2xl p-8 md:p-10 fade-up-2">
                ...
                <form method="POST" action="/kiosk/camera" class="space-y-5">
                    ...
                    class="w-full mb-4 rounded-xl bg-white/5 border border-white/10 px-4 py-3 text-white">

                    <div id="staffList"
                    class="grid sm:grid-cols-2 gap-3 max-h-64 overflow-y-auto">

                    @foreach($staffs as $staff)
                    <div
                    data-name="{{ strtolower($staff->name) }}"
                    onclick="selectStaff({{ $staff->id }}, '{{ $staff->name }}', this)"
                    class="staff-card cursor-pointer p-4 rounded-xl border border-white/10 bg-white/5 hover:bg-blue-500/20 transition">

                    <div class="flex items-center gap-3">

                    <div class="w-10 h-10 rounded-full bg-blue-500/20 flex items-center justify-center text-blue-300 font-bold">
                    ...
                    <p id="staffSelected"
                    class="hidden text-blue-400 text-sm mt-3">
                    Staff dipilih: <span class="font-bold"></span>
                    </p>
                    </div>
...
function selectStaff(id, name, el){
    ...
    const selected = document.getElementById('selectedStaff');
